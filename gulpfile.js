 ...
 */
var PIXI_PATH = './node_modules/pixi.js/bin/';
...
/**
 * Copies required PIXI files from the './node_modules/PIXI' folder into the './build/scripts' folder.
 * This way you can call 'npm update', get the lastest PIXI version and use it on your project with ease.
 */
function copyPIXI() {

    var srcList = ['pixi.min.js', 'pixi.min.js.map'];

    if (!isProduction()) {
        srcList.push('pixi.js.map', 'pixi.js');
    }

    srcList = srcList.map(function(file) {
        return PIXI_PATH + file;
    });
 ...
 *
 * In order to avoid copying PIXI and Static files on each build,
...
gulp.task('copyPIXI', ['copyStatic'], copyPIXI);
gulp.task('build', ['copyPIXI'], build);
gulp.task('fastBuild', build);
gulp.task('serve', ['build'], serve);
gulp.task('watch-js', ['fastBuild'], browserSync.reload); // Rebuilds and reloads the project when executed.
gulp.task('watch-static', ['copyPIXI'], browserSync.reload);

/**
 * The tasks are executed in the following order:
 * 'cleanBuild' -> 'copyStatic' -> 'copyPIXI' -> 'build' -> 'serve'
 *
